...
there were quite a few changes which were introduced to Rasa NLU and Rasa Core. On 30th of August, Rasa v.0.11 was released with a lot of changes under the hood. This repo contains the updated bot code compatible with the latest releases of Rasa Core and Rasa NLU.
...
The biggest change in how Rasa Core model works is that custom action 'action_' now needs to run on a separate server. That server has to be configured in a 'endpoints.yml' file.  This is how to train and run the dialogue management model:  
